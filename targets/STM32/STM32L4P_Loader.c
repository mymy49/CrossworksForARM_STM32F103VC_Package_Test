...
#define DUALBANK() ((FLASH_SIZE==0x200000) ? (FLASH->OPTR & FLASH_OPTR_DBANK) : (FLASH->OPTR & FLASH_OPTR_DB1M))
...
flash_erase_sector(libmem_driver_handle_t *h, libmem_sector_info_t *si)
{
  ...
  if (DUALBANK() && ISBANK2(si->number))
    FLASH->CR = FLASH_CR_PER | (si->number << 3) | FLASH_CR_BKER;
  else
